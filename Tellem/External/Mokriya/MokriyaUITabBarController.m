...
@synthesize allViewControllers,hasNotifications;
...
- (void)customizeTabBar:(NSUInteger)selectedTabIndex
{
    [self customizeTabBarImages];
    [self customizeTabBarLabels];
    [self didSelectTab:[tabBarButtonsArray objectAtIndex:selectedTabIndex]];
...
- (void)customizeTabBarImages
{
    ...
    for (int i=0;i < [self.viewControllers count]; i++) {
       ...
       // UIImage *buttonSelectedStateImage = [tabBarSelectedStateImagesArray objectAtIndex:i];
        ...
        tabBarButton.frame = CGRectMake(0.0, 0.0, tabWidth+2.0,tabHeight);
        ...
        if (i==0 && self.hasNotifications) {
            UIButton *removeViewButton = [UIButton buttonWithType:UIButtonTypeCustom];
            [removeViewButton setFrame:CGRectMake(tabBarButton.bounds.origin.x+tabBarButton.bounds.size.width-20, 5.0f, 15.0f, 15.0f)];
            [removeViewButton setBackgroundColor:[UIColor clearColor]];
            [[removeViewButton titleLabel] setAdjustsFontSizeToFitWidth:YES];
            [removeViewButton setBackgroundImage:[UIImage imageNamed:@"asterisk.png"] forState:UIControlStateNormal];
            [removeViewButton setSelected:NO];
            [tabBarButton addSubview:removeViewButton];
            [tabBarButton setNeedsDisplay];
        }
    }
}
...
- (void)customizeTabBarLabels
{
    ...
    for (int i=0;i < [self.viewControllers count]; i++) {
        ...
        tabBarLabel.font = [UIFont fontWithName:kFontThin size:10];
